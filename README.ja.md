...
# 🦁 XXD Panel 062｜賢くて不器用な線

### ソース画像の素朴な黒の線とアクセントカラーを使用して、写真のスマートな瞬間を保存します
...
写真の魅力が、ごく小さな動作や表情、関係性に宿っていることがあります。すべてを丁寧に描き込むとその一瞬が細部に埋もれ、きれいなベクターにすると人の手の感触やユーモアが失われます。色を増やしすぎれば、本当に記憶に残すべき一点も目立たなくなります。

**Panel 062** は、複雑な場面を一つの核となる図形と少数の記号へ圧縮します。少し不器用な黒線で主体らしさを保ち、原写真から最も生命力のあるアクセントカラーを一色だけ残します。洗練されたレイアウトが不完全さを支えるため、遠目には抑制が利き、近くで見ると親しみを感じる仕上がりになります。

### このような場面に適しています

- **複雑な写真から気の利いた一瞬だけを抽出したい：** 主体らしさと重要な関係は残し、背景や二次的な細部は思い切って引かせたい。
- **素朴な線が好きだが、子どもの落書きにはしたくない：** 線の揺れ、途切れ、描き直しの跡を、成熟したスケール感、重心、余白と共存させたい。
- **色を記憶の目印にしたい：** 画面全体で原写真のアクセントカラーを一色だけ残し、多色のパレットに焦点を奪わせたくない。
...
> XXD Panel 062 でこの画像を処理して、まず最適な構図とサイズを提案して。
...
**キーワード:** 子供っぽい黒のライン · 単一のアクセントカラー · 賢くて不器用 · 非常に明るい背景 · エディトリアルブランク

Panel 062 は、複雑な写真をコア グラフィックと少数の補助シンボルに圧縮します。不安定な黒の手描きの線画、広い面積の明るい白空間、ソース画像の特徴的な色を用いて、遠目では抑制され、近くでは親しみやすい編集画面を形成しています。

<!-- xxd-panel-benefit:start -->
## すぐに判断：XXD Panel 062 はあなたに合う？

| 知りたいこと | このスタイルの答え |
|---|---|
| **得られる完成作品** | ソース画像の素朴な黒の線とアクセントカラーを使用して、写真のスマートな瞬間を保存します |
| **ひと目で分かる特徴** | 子供っぽい黒のライン · 単一のアクセントカラー · 賢くて不器用 · 非常に明るい背景 · エディトリアルブランク |
...
複雑な写真を、すぐに認識できる 1 つの核となる形式といくつかの補助マークに圧縮します。不安定な黒の描画はアイデンティティと関係性を維持しますが、ソースから記憶された単一の鮮やかな色がアクセントをもたらします。プロフェッショナルな間隔は、意図的な不完全性をサポートします。

```text
アイデンティティ、輪郭、キーの関係をロックする → それらを 1 つの核となる形式に圧縮する → ぐらつき、切れ目、修正マークによって黒い線を構築する → ソースからの 1 つの記憶色を保持する → スケールのコントラストとスペースを通じて専門的な順序を確立する → 手書きの観察の痕跡のみを追加する
```

## 完成作品を見分けるポイント

- 1 つの核となる形式といくつかの必要なシンボルが全体のストーリーを伝えます。複雑なシーンは意図的に削減されています。
- 黒い線は、洗練されたデジタル線画になるのではなく、揺れ、壊れ、探し、素朴に誇張されます。
- ソース由来のアクセントカラーが 1 つだけ、記憶に残る鮮やかな細部のように、非常に小さな領域に表示されます。
- 被写体は中心から外れて座ったり、浮いたり、低く休んだりする一方で、大きな空白の領域が積極的に構図を形作ります。
- 遠くから見ると、抑制的でプロフェッショナルに見えます。近くで見ると、温かく奇妙で、明らかに修正されているように感じられます。意図された知的なぎこちなさです。

美的制約と避けるべき表現は[中国語の原文プロンプト](references/original-prompt/zh-CN.md)だけにあります。Skill と実行アダプターは今回の納品条件だけを扱います。 [Skill ワークフロー](SKILL.md) · [英語ランタイムアダプター](references/xxd-panel-062-prompt.en.md)
...
画像フォルダーを直接渡すと、この兵士 Skill が同じ Panel 062 の原文美学で一括処理します。一般的なラスター形式を既定で再帰検索し、件数を報告してから、モード・サイズ・文字・言語を一度だけ確認します。各画像は内容や文言を混ぜず独立生成し、バッチ全体は一つのフラットなタスクフォルダーへ保存します。個別の失敗は記録し、後続画像を黙って省略しません。
...
/xxd-panel-062 photo.jpg --mode top-bottom --size auto --text prompt --locale ja-JP
...
/xxd-panel-062 photo.jpg --mode design-only --size 3:4 --text prompt --locale ja-JP

# 正確な文言を、書き換え・翻訳・タイトル追加なしで使用
/xxd-panel-062 photo.jpg --mode design-only --size 3:4 --copy "光をここに残す" --locale ja-JP

# 2モード × 3サイズで独立した完成構図を6枚生成
/xxd-panel-062 photo.jpg --mode top-bottom,design-only --size 1:1,3:4,9:16 --text prompt --locale ja-JP

# フォルダー全体へ Panel 062 を適用し、共通設定は一度だけ解決
/xxd-panel-062 "/path/to/photos" --mode design-only --size auto,9:16 --text prompt --locale ja-JP
...
Claude Code では同じフォルダを次へリンクできます： `~/.claude/skills/xxd-panel-062`. インストール後に Agent セッションを再起動してください。
...
完全仕様: [Skill ワークフロー](SKILL.md) · [原始スタイル資料](references/original-prompt/zh-CN.md) · [英語ランタイムアダプター](references/xxd-panel-062-prompt.en.md) · [中国語ランタイムアダプター](references/xxd-panel-062-prompt.zh-CN.md)
...
| **[xxd-panel-062](https://github.com/nevertoday/xxd-panel-062)** | 黒い主形 · 巨大な余白 · 網点の消散 · 禅的思考 · 思考の断片 |
...
| **兵士級** | `xxd-panel-NNN`（現在 001–062） | 各番号が固有の原文プロンプトと美学だけを実行し、将軍から渡された一つの仕事を完成させます。 |
