...
**Panel 062** 适合这类任务：需要保留原图核心内容，同时希望最终视觉达到以下方向：**用稚拙黑线和一枚源图强调色保留照片的聪明瞬间**。它先从用途与期望结果出发，再决定构图、尺寸、文字和具体输出形式。
...
把复杂照片压缩成一个一眼可辨的核心图形和少量辅助符号，以不稳定的黑色手绘线保住身份与关系，再只留下原图中一枚最有生命力的颜色。专业版式负责托住这种故意的不完美。

```text
锁定身份、轮廓与关键关系 → 压缩为一个核心图形 → 用抖动、断开与修正痕迹建立黑线 → 从原图保留一枚记忆色 → 以尺度反差和大留白建立专业秩序 → 加入极少量手写观察
```

## 成品中最容易识别的风格特点

- 一个核心图形与少量必要符号承担全部叙事，复杂场景被主动删减。
- 黑线有轻微抖动、断开、重复寻找和天真比例，不变成整洁数字线稿。
- 全画面只使用一枚源图强调色，面积很小，像记忆突然保留下来的亮点。
- 主体可偏心、悬置或落在低重心位置，大量空白主动参与构图。
- 远看克制而专业，近看亲切、古怪、有修正痕迹，形成“聪明的笨拙感”。

原始审美约束与拒绝项只存在于[原始提示词](references/original-prompt/zh-CN.md)；Skill 与运行适配器只处理本次交付变量。 [Skill 工作流](SKILL.md) · [英文运行适配器](references/xxd-panel-062-prompt.en.md)
...
| **[xxd-panel-062](https://github.com/nevertoday/xxd-panel-062)** | 黑色主形 · 极大留白 · 网点消散 · 禅意思考 · 哲思碎片 |
...
| **士兵级** | `xxd-panel-NNN`（当前 001–062） | 每个编号只执行自己独立的原始提示词与审美，把将军派发的单个任务完成为成品。 |
