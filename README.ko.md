...
어떤 사진은 아주 작은 동작이나 표정, 관계에서 매력이 생깁니다. 모든 것을 충실히 그리면 그 순간이 디테일에 묻히고, 깔끔한 벡터로 만들면 사람 손의 감각과 유머가 사라집니다. 색이 많아질수록 정말 기억해야 할 한 가지 포인트도 두드러지지 않습니다.

**Panel 062**는 복잡한 장면을 하나의 핵심 형태와 소수의 기호로 압축하고, 조금 서툰 검은 선으로 정체성을 지킨 뒤 원본에서 가장 생명력 있는 강조색 하나만 남깁니다. 전문적인 레이아웃이 불완전함을 받쳐 주어, 멀리서는 절제되고 가까이서는 친근하게 보입니다.

### 이런 상황에 적합합니다

- **복잡한 사진에서 영리한 한순간을 뽑아내고 싶을 때:** 주인공의 정체성과 핵심 관계는 남기고, 배경과 부차적인 디테일은 적극적으로 덜어낼 수 있습니다.
- **서툰 선은 좋지만 어린이 낙서처럼 보이고 싶지 않을 때:** 떨림, 끊긴 선, 수정 흔적이 성숙한 크기 조절과 중심, 여백과 공존해야 합니다.
- **색을 기억의 포인트로 만들고 싶을 때:** 화면 전체에 원본 강조색 하나만 남겨 다색 팔레트가 초점을 빼앗지 않게 합니다.
...
> XXD Panel 062으로 이 이미지를 작업하고, 먼저 가장 어울리는 구성과 크기를 추천해 줘.
...
## 빠른 판단: XXD Panel 062이 나에게 맞을까요?

| 궁금한 점 | 이 스타일이 주는 답 |
|---|---|
| **얻게 되는 결과** | 사진의 스마트한 순간을 보존하려면 원본 이미지의 자연스러운 검정색 선과 강조 색상을 사용하세요 |
| **한눈에 알아볼 특징** | 유치한 검정색 선 · 단일 악센트 색상 · 영리하고 서투른 · 매우 밝은 배경 · 편집 공백 |
...
복잡한 사진을 즉시 인식할 수 있는 하나의 핵심 형태와 몇 가지 지지 표시로 압축합니다. 불안정한 검정색 드로잉은 정체성과 관계성을 유지하고, 출처에서 기억되는 단 하나의 생생한 색상은 악센트를 담고 있습니다. 전문적인 간격은 의도적인 불완전성을 뒷받침합니다.

```text
정체성, 윤곽, 주요 관계 잠금 → 하나의 핵심 형태로 압축 → 흔들림, 끊김 및 수정 표시를 통해 검은 선 만들기 → 원본에서 하나의 기억 색상 유지 → 규모 대비 및 공간을 통해 전문적인 질서 확립 → 손으로 쓴 관찰의 흔적만 추가
```

## 완성 스타일을 알아보는 특징

- 하나의 핵심 형태와 몇 가지 필수 기호가 전체 이야기를 전달합니다. 복잡한 장면은 의도적으로 줄였습니다.
- 검은 선은 세련된 디지털 라인 아트가 되기보다는 순진하게 흔들리고, 깨지고, 검색되고, 과장됩니다.
- 기억에 남아있는 생생한 디테일처럼 아주 작은 영역에 소스에서 파생된 단 하나의 액센트 컬러만 나타납니다.
- 넓은 빈 공간이 적극적으로 구도를 형성하는 동안 피사체는 중앙에서 벗어나거나 떠 있거나 낮게 놓여 있을 수 있습니다.
- 멀리서 보면 절제되고 전문적인 느낌이 듭니다. 가까이서 보면 따뜻하고 이상하며 눈에 띄게 수정된 느낌이 듭니다. 의도된 지능적인 어색함입니다.

미적 제약과 금지 항목은 [중국어 원본 프롬프트](references/original-prompt/zh-CN.md)에만 있습니다. Skill과 런타임 어댑터는 이번 전달 변수만 처리합니다. [Skill 워크플로](SKILL.md) · [영문 런타임 어댑터](references/xxd-panel-062-prompt.en.md)
...
이미지 폴더를 직접 전달하면 이 병사 Skill이 같은 Panel 062의 원본 미학으로 일괄 처리합니다. 일반 래스터 형식을 기본적으로 재귀 탐색하고 수량을 먼저 알린 뒤 모드·크기·텍스트·언어를 한 번만 확인합니다. 각 이미지는 내용이나 문구가 섞이지 않도록 독립 생성하며 전체 배치는 하나의 평면 작업 디렉터리만 사용합니다. 개별 실패는 기록하고 이후 이미지를 조용히 누락하지 않습니다.
...
/xxd-panel-062 photo.jpg --mode top-bottom --size auto --text prompt --locale ko-KR

# 좌우 비교: 3:2 가로형, 글자 없음
/xxd-panel-062 photo.jpg --mode left-right --size 3:2 --text none

# 디자인 전용: 9:16 휴대폰 세로형, 한국어 문구
/xxd-panel-062 photo.jpg --mode design-only --size 9:16 --text prompt --locale ko-KR
...
/xxd-panel-062 photo.jpg --mode design-only --size 3:4 --copy "빛을 이곳에 남겨 두기" --locale ko-KR

# 2개 모드 × 3개 크기로 독립적인 완성 구도 6개 생성
/xxd-panel-062 photo.jpg --mode top-bottom,design-only --size 1:1,3:4,9:16 --text prompt --locale ko-KR

# 폴더 전체에 Panel 062 적용, 공통 설정은 한 번만 확정
/xxd-panel-062 "/path/to/photos" --mode design-only --size auto,9:16 --text prompt --locale ko-KR
...
Claude Code 사용자는 같은 폴더를 다음 위치에 연결할 수 있습니다: `~/.claude/skills/xxd-panel-062`. 설치 후 에이전트 세션을 다시 시작하세요.
...
전체 사양: [Skill 워크플로](SKILL.md) · [원본 스타일 자료](references/original-prompt/zh-CN.md) · [영문 런타임 어댑터](references/xxd-panel-062-prompt.en.md) · [중문 런타임 어댑터](references/xxd-panel-062-prompt.zh-CN.md)
...
| **[xxd-panel-062](https://github.com/nevertoday/xxd-panel-062)** | 검은 주도형 · 거대한 여백 · 망점 소멸 · 선적 사유 · 생각의 파편 |
...
| **병사급** | `xxd-panel-NNN` (현재 001–062) | 각 번호가 고유한 원본 프롬프트와 미학만 실행해 장군이 배정한 하나의 작업을 완성합니다. |
